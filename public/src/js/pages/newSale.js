...
export async function initNewSale() {
    ...
    // Refresh dropdowns when services change
    onUpdate('services', () => populateServiceDropdown());

    // Listen for edit-sale events
        ...
        try {
            ...
            if (editingId) {
                ...
                if (existing) {
                    ...
                    sale.created_at = existing.created_at;
                }
            }
            ...
            submitBtn.textContent = editingId ? 'Atualizar Venda' : 'Registrar Venda';
        }
    });
}
...
async function loadSaleForEdit(sale) {
    ...
    // Trigger service change to fill supplier / commission
